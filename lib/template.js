...
class Template {
    ...
    async matchFromTask(task, input = {}) {
        const {
            match,
            matches,
            matcher,
    ...
    async extractFromTask(task, input = {}) {
        const {
            extract,
            extracts,
            extractor,
        ...
        let result = deepmerge({}, this.ensureObject(input))

        for (let e of this.ensureArray(extract || extracts || extractor || extractors)) {
            result = Object.assign(result, deepmerge(result, await this.extract(e, input)))
        }
    ...
    async runTask(taskName, task, input = {}) {
        input = await this.executeTask(taskName, await this.getTaskDefinition(task), input)
        ...
        if (await this.matchFromTask(task, input)) {
            matches = true
            extracts = await this.extractFromTask(task, input)

            output = deepmerge(output, extracts)
        }

        return { id: task.id, input, output, matches, extracts }
    }
