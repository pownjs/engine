...
class Template {
    ...
    // NOTE: While still supported the matcher functionality is now considered legacy. We are now using
    // the much more powerful mechanism based on the mingo library.

    getMatchConditionFunc(condition) {
        return {
            'all': asyncEvery,
            'any': asyncSome,
    ...
    async matchInternal(matcher, input = {}) {
        const { part, condition, lt, lessthan, gt, greaterthan, lte, lessthanorequal, gte, greaterthanorequal, eq, equal, number, numbers, string, strings, word, words, regex, regexes, flag, flags, b64, base64, b64s, base64s, entropy, script } = matcher
        ...
        switch (type) {
            case 'lt':
            case 'lessthan':
                return this.getMatchConditionFunc(condition)(ensureArray(eq || equal || number || numbers || NaN), (number) => {
            ...
            case 'words':
                if (this.isString(input) || this.isBuffer(input)) {
                    return this.getMatchConditionFunc(condition)(ensureArray(word || words || ''), (word) => {
            ...
            case 'regexes':
                if (this.isString(input) || this.isBuffer(input)) {
                    return this.getMatchConditionFunc(condition)(ensureArray(regex || regexes || ''), (regex) => {
            ...
            case 'base64':
                if (this.isString(input)) {
                    return this.getMatchConditionFunc(condition)(ensureArray(b64 || base64 || b64s || base64s || ''), (b64) => {
            ...
            case 'script':
                return this.getMatchConditionFunc(condition)(ensureArray(script || ''), (script) => {
    ...
    async match(matcher, input = {}) {
        ...
        if (negative || not) {
            return !this.matchInternal(rest, input)
        }
        else {
            return this.matchInternal(rest, input)
        }
    ...
    async matchWithTask(task, input = {}) {
        ...
        return this.getMatchConditionFunc(c)(m, (m) => this.match(m, input))
    }
