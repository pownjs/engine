...
class Template {
    ...
    async runTask(taskName, task, input, ...args) {
        ...
        if (error) {
            ...
            this.tracer.push(`Task execution finished`, { name: taskName, task, input, result, error })
    ...
    async * runIt(input = {}, ...args) {
        ...
        this.tracer.push(`Executing template`, { id, template: this.template })

        yield* this.runTaskDefinitionsIt(tasks, input, ...args)

        const finishedAt = Date.now() // TODO: get nanosecond precision

        this.tracer.push(`Template execution finished`, { id, template: this.template, startedAt, finishedAt })
    }
