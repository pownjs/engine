...
class Matcher {
    ...
    async test(input) {
        const { part, condition, word, words, regex, regexes, flag, flags, entropy, script } = this.matcher

        if (part && !isString(input)) {
            input = query(input, part || '')
        }

        let { type } = this.matcher
...
class Extractor {
    ...
    async extract(input) {
        const { part, name, value, jsonpath, path, regex, flag, flags, group, script } = this.extractor

        if (part && !isString(input)) {
            input = query(input, part || '$')
        }

        let { type } = this.extractor
...
const createSimpleTemplateHandler = (definition) => {
    class GenericTask extends Task {
        async run(name, ...args) {
            ...
            if (pre) {
                const postArgs = await pre.call(this, ...args)

                if (postArgs) {
                    args = getArray(postArgs)
                }
            }

            let results = await super.run(...args)

            if (post) {
                const postResults = await post.call(this, results, ...args)
    ...
    class EngineTemplate extends Template {
        async run(...args) {
            ...
            for (let [name, value] of Object.entries(this.template)) {
                if (['op', 'ops', 'operation', 'operations'].includes(name)) {
                    const ots = new OperationTaskSet(getArray(value))
